...
- **Recommended code change (low-risk, ready to implement without a live device):**
  1. Change `verify_tls: bool` type hints to `Union[bool, str]` in `pan_auth.py`, `pan_base.py`, `main.py` — no logic change needed, since `requests` already handles a string path correctly when passed straight through as `verify=`.
