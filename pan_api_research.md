...
- **Updated recommended account configuration (corrected against actual runtime behavior — grepped every `type=` value the code sends: only `op` and `log` are ever issued; `_xml_config_get` exists in `pan_base.py` but nothing calls it; `report` is never referenced at all):**
  - XML API tab: enable only **Op, Log**; leave Commit/Config/Export/Import/IoT/Report/User-ID disabled. (Superseded: an earlier pass here said "Report, Log, Config" — that never matched what the code actually sends and should not be used.)
  - REST API tab: set **`Policies.SecurityRules`, `Objects.UrlFilteringSecurityProfiles`, `Objects.AntiSpywareSecurityProfiles`** individually to Read Only; leave every other REST object/rule type Disabled. **`SecurityPreRules`/`SecurityPostRules` are not separate toggles** — the `pango` schema's Policies category has exactly 11 fields and only one of them is rule-related (`SecurityRules`); there's no distinct Pre/Post-Rules permission at the RBAC layer even though they're distinct REST *resources*. Real-world-confirmed: looked for them in the Panorama Admin Role Profile UI (both firewall- and Panorama-level) and they don't exist there either — consistent with this finding, not a UI bug or a missed tab.
